...
const PrizeSplit = ({item, withBalances = true}) => {
    ...
    return <React.Fragment>
        {withBalances ? item.awardBalance.plus(awardSponsorBalance).div(item.numberOfWinners).toFixed(2) : ''} {item.token}
        {sponsors.map((sponsor, i) => {
            return <React.Fragment key={item.id + i}>{i < sponsors.length - 1 ? ", " : " & "}
