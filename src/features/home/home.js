    ...
    <React.Fragment>
      <Container maxWidth="xl">
        <Typography className={classes.h1}>
          <Trans
            i18nKey="homeTitle"
            values={{ amount: Number(vault.totalPrizesAvailable.toFixed(0)).toLocaleString() }}
          />
        </Typography>
        <Box>
          <Filter
            ...
            {filtered.length === 0
              ...
              : filtered.map(item => (
                  <React.Fragment key={item.id}>
                      ...
                      <Grid container>
                        ...
                        <Grid item xs={8}>
                          ...
                          <Typography className={classes.potUsd} align={'right'}>
                            <span>{t('in')}</span> {item.token} <span>{t('and')}</span>{' '}
                            {item.sponsorToken}
                          </Typography>
                          <Typography className={classes.potCrypto} align={'right'}>
                            USD {t('value')} {t('prize')}
                          </Typography>
                        </Grid>
                        ...
                        <Grid item xs={6} style={{ paddingLeft: '8px' }}>
                          ...
                          <Typography className={classes.apy} align={'right'}>
                            <span>{item.apy}%</span>{' '}
                            {item.bonusApy > 0
                        ...
                        <Grid item xs={9} align={'left'} style={{ paddingBottom: 0 }}>
                          <Typography
                            className={classes.prizeSplitText}
                            onClick={() => {
                          ...
                          <AnimateHeight duration={500} height={prizeSplitOpen ? 'auto' : 0}>
                            <Grid container spacing={1}>
                              <Grid item xs={3} align={'left'}>
                                <Typography className={classes.prizeSplitWinners}>
                                  5 winners
                                </Typography>
                              </Grid>
                              <Grid item xs={9} align={'right'}>
                                <Typography className={classes.prizeSplitValue}>
                                  <span>
                                    {item.awardBalance.times(0.2).toFixed(2)} {item.token}
                                  </span>{' '}
                                  and{' '}
                                  <span>
                                    {item.sponsorBalance.times(0.2).toFixed(2)} {item.sponsorToken}
                                  </span>{' '}
                                  each
                                </Typography>
                              </Grid>
                        ...
                        <Grid item xs={12}>
                          <Button
                            ...
                            variant={'contained'}
                            onClick={() => {
                              history.push('/pot/' + item.id);
                            }}
                          >
                            {t('buttons.playWith')} {item.token}
                          </Button>
                        </Grid>
                        <Grid item xs={8}>
                          <Typography className={classes.oddsPerDeposit}>
                            {t('oddsPerDeposit', {
                              odds: investmentOdds(
                                item.totalStakedUsd,
                                BigNumber(1000),
                    ...
                    <Grid item xs={12}>
                      <Typography className={classes.beefy}>
                        {t('footerPoweredBy')}{' '}
                        <Link href={'https://beefy.finance'}>Beefy.Finance</Link>
                        <img
                          alt="Beefy Finance"
                          srcSet="
                ...
                <Grid container>
                  <Grid item xs={12}>
                    <Box className={classes.ziggyMaintenance}>
                      <img
                        alt="Ziggy Maintenance"
                        height="100px"
                        src={require('../../images/ziggy/maintenance.svg').default}
                      />
                    </Box>
                  </Grid>
                  <Grid item xs={12}>
