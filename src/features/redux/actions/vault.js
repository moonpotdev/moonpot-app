...
const getPools = async (items, state, dispatch) => {
  ...
  for (let key in items) {
    ...
    if (Number(pool.sponsorRewardId) >= 0) {
      call.sponsorRewardInfo = gateContract.methods.rewardInfo(pool.sponsorRewardId);
    }

    if (Number(pool.boostRewardId) >= 0) {
      call.boostRewardInfo = gateContract.methods.rewardInfo(pool.boostRewardId);
    }

    calls[pool.network].push(call);
